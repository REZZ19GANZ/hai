...
conn.setStatus(`Status : ${opts.disable ? 'Una' : 'A'}vailable | Uptime : ${kyun(process.uptime())} | User registered : ${registered.length} | Battery : ${batere}% | Auto update bio after 30 Seconds`)
}
}, 30 * 1000)
...
if (opts['test']) {
  ...
  conn.connect().then(() => {
 conn.sendMessage('6281313726400@c.us', 'Bot Successfully Running', 'conversation');
